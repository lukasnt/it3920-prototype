...
class CSVTemporalGraphLoader(val temporalParser: TemporalParser = new TemporalParser {
  override def parse[T <: Temporal](temporal: String): T =
    DateTimeFormatter.ISO_DATE_TIME
      .parse(temporal)
      .asInstanceOf[T]
}, val separator: String = "|", val startDateColumn: String = "creationDate", val endDateColumn: String = "deletionDate", val srcIdColumn: String = "srcId", val dstIdColumn: String = "dstId")
    extends TemporalGraphLoader {

  override def readEdgeListFiles[T <: Temporal](sc: SparkContext, labelFiles: Map[String, String]): TemporalGraph[T] = {
    ...
    for ((label, file) <- labelFiles) {
      ...
      for (line <- lines) {
        ...
        val properties = header
          .zip(columns)
          .filterNot(column => column._1 == startDateColumn || column._1 == endDateColumn)
          .toMap
